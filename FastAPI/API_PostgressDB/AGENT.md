...
- Don't write a `try`/`except` whose block only logs, re-raises, wraps, or hand-builds a 500 — the `exception_handler` middleware already does that once for every route. Catch only to translate a failure (to a result enum, to `None`) or to undo work (rollback), and never to swallow an error behind a fake success.
