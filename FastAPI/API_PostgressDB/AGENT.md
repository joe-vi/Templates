...
│   └── database/{base.py, session.py, errors.py, connection_factory.py, sqlalchemy_connection_factory.py, sqlalchemy_transaction_context.py, models/<entity>_model.py}
│                  ├── errors.py    # shared driver-error classifiers (is_deadlock) reused by every repository adapter
│                  ├── connection_factory.py            # ConnectionFactory seam: read() (fresh short-lived session) / write() (the request write unit of work)
│                  └── sqlalchemy_transaction_context.py # owns the write session per outermost begin(); auto-commit on clean exit; fail-fast on a poisoned unit
└── api/
...
- **Scopes are explicit**, and chosen by *what holds request state*: `singleton` (from `injector`) for process-wide stateless objects (engine, `async_sessionmaker`, `PasswordHasher`, `TokenService`); `request` (from `src/infrastructure/di/request_scope.py`) for objects that carry per-request state (the write unit of work `SqlAlchemyTransactionContext`, the bound `Logger`, and `UserContext`); and **transient** (no scope) for stateless orchestrators that only wire injected deps — the **use cases, repositories, and the `ConnectionFactory` adapter**. The transaction context is request-scoped and bound to **both** itself and the `TransactionContext` port through two `@request @provider` methods that return the same instance, so a use case's `begin()` and a repository's `ConnectionFactory.write()` nest on one write session. There is **no** request-scoped `AsyncSession`.
...
- **Disposal**: on request end the scope disposes its objects in reverse creation order — `aclose()` preferred, an async `close()` is awaited, failures are logged without blocking other teardowns (the bound `Logger` and `UserContext` go this way). **Sessions are not disposed by the scope**: the write session is created and closed inside each outermost `begin()`, and a `read()` session closes on its own block exit, so the request-scoped `SqlAlchemyTransactionContext` holds no session between scopes. The engine (a singleton) is disposed explicitly in `main.lifespan` shutdown.
...
- The engine and `async_sessionmaker` are **singleton `@provider` methods** on `AppModule`; the engine is disposed in `main.lifespan` shutdown. There is **no** request-scoped `AsyncSession` — nothing shares one session across a whole request.
- **Repositories get sessions through a `ConnectionFactory`** (`src/infrastructure/database/connection_factory.py`; adapter `SqlAlchemyConnectionFactory`) — an **infrastructure-internal seam, not a hexagonal port**: it exposes an `AsyncSession` directly (so it stays SQLAlchemy-coupled and is consumed only by repositories), and lives in `src/infrastructure/`, not `src/ports/`. Two lifetimes:
  - `read()` yields a **fresh short-lived session**, closed as soon as its block exits, returning the pooled connection immediately (no lingering idle-in-transaction).
  - `write()` yields the **request write unit of work** — the session owned by `SqlAlchemyTransactionContext` — opening it, or (within the same asyncio task) joining the one already open.
- **The write unit of work is owned by `SqlAlchemyTransactionContext`** (`src/ports/transaction_context.py`; adapter in `src/infrastructure/database/`), injected with the `async_sessionmaker`, **not** a session. The active unit lives in a **per-task `contextvars.ContextVar[_WriteUnit | None]`** (`_WriteUnit` = the session plus a `rolled_back` flag) held on the instance, so concurrent asyncio tasks sharing one context each get their own session and reentrancy is per task.
  - **Outermost `begin()`**: create a session, `await session.begin()`, set the ContextVar; **auto-commit on a clean exit**, **rollback on any escaping exception**, always `close()`.
  - **Nested `begin()`** (same task, unit already open): **join** the unit — do not create/commit/close. If a write raises inside it, roll the **whole** unit back and re-raise.
  - **Fail-fast on a poisoned unit**: once a unit is rolled back it is dead — a further `begin()` in it **raises**, so no write runs on an aborted transaction and no caller sees a success that can never commit.
- **Auto-commit, not explicit commit.** `Transaction` has **no `commit()`** — the outermost scope commits on a clean exit. A **single** repository write self-commits (its own `write()` is the outermost `begin()`), so a **single-write use case needs no transaction context at all**; it calls the repository directly. A use case injects `TransactionContext` **only** to make **several** writes atomic, wrapping them in `async with self._transaction_context.begin():`. Note an early `return` inside `begin()` now **commits** unless you rolled back first.
- **Explicit `rollback()` only for a benign failure.** A DB error is auto-detected (it raises and rolls the unit back); a benign failure **result** that raises no exception (e.g. `NOT_FOUND` from `rowcount == 0`) is **not**. In a multi-write use case, `await transaction.rollback()` before returning if such a result must abort the unit.
- **Reads happen before the write unit, never inside it.** A repository read goes through `ConnectionFactory.read()` on its own short-lived session, so it never joins the write transaction — a `begin()` block must contain **only writes**. Do all loading (and any decision it drives) first, then open `begin()` for the writes. Never call a repository read inside a `begin()` block: it checks out a second pooled connection while the write connection is held (a needless second connection, and a deadlock risk under load). A load → behave → persist flow reads the aggregate first, applies the entity's rule, then persists (a single persist self-commits and needs no `begin()`).
- Repository **adapters inject `ConnectionFactory`**. Wrap reads in `async with self._connections.read() as session:` and writes in `async with self._connections.write() as session:`. They **never commit or roll back** — the context owns the boundary. Mutations `flush()` (inserts) or `execute()` (update/delete) so DB errors surface and are mapped to result enums; reads just query. Put the `try`/`except` **outside** the `write()` block, so the context rolls the unit back and marks it dead **before** the repository translates the error to a result enum.
...
- **Atomic multi-repository operations**: call any number of repository **writes** inside one `begin()` block; they share the unit's session, so they all commit together on a clean exit or all roll back on the first failure (a DB error rolls the unit back automatically; call `rollback()` for a benign non-success result).
...
- **`BackgroundTasks` and streaming bodies** run inside the request scope (see §4 Middleware). Their own DB work is a *new* unit of work and must open its own `write()` / `begin()`; anything genuinely separate belongs in a task queue, not in `BackgroundTasks`.
- **Reads no longer pin the pooled connection.** Because `read()` closes its session on block exit, the `SELECT`'s autobegun transaction ends and the connection returns to the pool immediately — so the old read-route background-task trap is largely dissolved and reads do **not** need a `begin()` wrapper. Writes still commit (freeing the connection) before a background task starts, so a background task inherits a clean session.
...
- **`RequestScopeMiddleware` is deliberately the exception: a pure ASGI class** (`__call__(self, scope, receive, send)`, registered with `app.add_middleware(...)`). **Do not "tidy" it into a dispatch function** — that silently reintroduces a use-after-disposal bug. `BaseHTTPMiddleware.call_next` returns as soon as the response *starts*, so the scope would dispose its request-scoped collaborators (the write unit of work `SqlAlchemyTransactionContext`, the bound `Logger`, the `UserContext`) while `BackgroundTasks` and streaming response bodies are still running. It fails silently rather than loudly: anyio copies the context when it spawns the downstream task, so those callers still resolve the *same, already-disposed* instances. A pure ASGI middleware wraps the raw `await self._app(...)`, which does not return until background work has finished. Regression tests live in `tests/api/middleware/test_request_scope_middleware.py`.
...
2. **Infrastructure**: ORM model in `src/infrastructure/database/models/<entity>_model.py` (re-export from `models/__init__.py`); adapter `sqlalchemy_<entity>_repository.py` subclassing the port and taking a `ConnectionFactory` (wrap reads in `read()`, writes in `write()`).
3. **Application**: `ContractModel` request/response contracts (with validation) in `<entity>_contracts.py`, converter **functions**, one concrete use case class per operation (single `execute` method) in `src/application/use_cases/<entity>/`. A **single-write** use case calls the repository directly (its `write()` self-commits — no transaction context). A **multi-write** use case injects `TransactionContext` and wraps the writes in one `begin()` block (auto-commit on clean exit; `rollback()` to abort on a benign non-success result).
...
- A **single-write** use case takes no transaction context: its test just asserts the repository result is forwarded.
- A **multi-write** use case injects `TransactionContext`: give its test a tiny local `FakeTransactionContext` (an async-context-manager fake — commit-on-clean-exit, rollback on exception or after `rollback()`) and assert it is **not rolled back on success** and **rolled back on failure** (there is no `commit()` to assert). The template ships no multi-write use case today, so it carries no such fake; unit-of-work behaviour itself is covered by the integration test in `tests/infrastructure/database/`.
...
| Infrastructure | repository adapter | Yes — integration test over a real `SqlAlchemyConnectionFactory` on aiosqlite (or a `FakeConnectionFactory`); unit-of-work behaviour is exercised here |
| Infrastructure | `SqlAlchemyTransactionContext` | Yes — integration test against in-memory SQLite (aiosqlite) proving auto-commit / rollback atomicity **and** the poisoned-unit fail-fast (a DB error rolls the whole unit back and every following write fails fast) |
| Infrastructure | `di/` (request scope, typed binder) | Yes — unit tests in `tests/infrastructure/di/` |
| Architecture | all of `src/` | Yes — a fitness test in `tests/architecture/` parses every module's imports (AST) and fails on any breach of the inward-only dependency direction or the framework-purity of Domain/Ports |

**Architecture fitness test.** `tests/architecture/test_layer_dependencies.py` is the executable guardrail for §1: it walks `src/`, and for each module asserts (a) it imports only from the layers the dependency table permits (Domain from nothing, Ports from Domain/`shared`, Application from Domain/Ports/`shared`, Infrastructure inward of API, ...), and (b) Domain and Ports pull in **no** third-party package while Application never imports `sqlalchemy`/`fastapi`. A new cross-layer import that violates the direction fails this test with the offending file named — so the architecture cannot silently rot. Extend its rule tables when you add a legitimately new layer or allowed edge.
...
- Don't keep session (or other control-flow) state in a **module-global** `ContextVar`. Repositories get their session through the injected `ConnectionFactory` (`read()` / `write()`); the transaction context's per-task `ContextVar` is an **instance** member of the request-scoped adapter, not module-global ambient state.
- Don't pass an `AsyncSession` to a use case or a repository constructor — repositories get sessions through the injected `ConnectionFactory`; use cases never touch a session.
- Don't commit or roll back inside repositories — the transaction context owns the boundary (auto-commit on clean exit, rollback on exception).
- Don't call `commit()` from a use case — there is **no** `commit()`. The outermost `begin()` commits on a clean exit; abort with `await transaction.rollback()` (benign non-success result) or by letting an exception propagate.
...
- Don't rewrite `RequestScopeMiddleware` as a `BaseHTTPMiddleware` dispatch function — `call_next` returns when the response starts, so the scope would dispose its request-scoped collaborators (the write unit of work, the `Logger`, the `UserContext`) under background tasks and streaming bodies, which still resolve the same disposed instances.
