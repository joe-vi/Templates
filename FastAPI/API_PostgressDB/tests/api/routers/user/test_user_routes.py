...
class TestCreateUserRoute:
    async def test_returns_201_with_result_and_id_on_success(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.create_user.return_value = (operation_results.CreateResult.SUCCESS, 1)
    ...
    async def test_returns_409_on_unique_constraint_error(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.create_user.return_value = (operation_results.CreateResult.UNIQUE_CONSTRAINT_ERROR, None)
    ...
    async def test_returns_409_on_concurrency_error(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.create_user.return_value = (operation_results.CreateResult.CONCURRENCY_ERROR, None)
    ...
    async def test_returns_500_on_failure(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.create_user.return_value = (operation_results.CreateResult.FAILURE, None)
...
class TestUpdateUserRoleRoute:
    async def test_returns_200_on_success(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.update_user_role.return_value = operation_results.UpdateResult.SUCCESS

        response = await client.patch("/api/v1/users/1/role", json={"role": user_enum.UserRole.ADMIN})

        assert response.status_code == 200
        assert response.json()["result"] == operation_results.UpdateResult.SUCCESS

    async def test_returns_404_when_user_not_found(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.update_user_role.return_value = operation_results.UpdateResult.NOT_FOUND

        response = await client.patch("/api/v1/users/99/role", json={"role": user_enum.UserRole.ADMIN})

        assert response.status_code == 404

    async def test_returns_409_on_concurrency_error(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.update_user_role.return_value = operation_results.UpdateResult.CONCURRENCY_ERROR
    ...
    async def test_returns_500_on_failure(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.update_user_role.return_value = operation_results.UpdateResult.FAILURE
...
class TestDeleteUserRoute:
    async def test_returns_200_on_success(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.delete_user.return_value = operation_results.DeleteResult.SUCCESS
    ...
    async def test_returns_404_when_user_not_found(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.delete_user.return_value = operation_results.DeleteResult.NOT_FOUND
    ...
    async def test_returns_409_on_concurrency_error(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.delete_user.return_value = operation_results.DeleteResult.CONCURRENCY_ERROR
    ...
    async def test_returns_500_on_failure(self, client: AsyncClient, mock_use_case: AsyncMock):
        mock_use_case.delete_user.return_value = operation_results.DeleteResult.FAILURE
