"""Integration tests for SqlAlchemyTransactionContext.

Runs against in-memory SQLite (no external services) to verify the unit-of-work
guarantee with a real database: every repository operation inside one begin()
block commits together or rolls back together.
"""
...
async def _count_users(
    ...
    async with session_factory() as session:
        query_result = await session.execute(
            select(func.count()).select_from(user_model.UserModel)
        )
        return query_result.scalar_one()


class TestCommit:
    async def test_committed_operations_are_persisted(
        ...
        async with session_factory() as session:
            ...
            async with transaction_context.begin() as transaction:
                result, user_id = await repository.create(
    ...
    async def test_flush_populates_id_before_commit(
        self, session_factory: async_sessionmaker[AsyncSession]
    ):
        async with session_factory() as session:
            repository = SqlAlchemyUserRepository(session)
...
class TestRollback:
    async def test_block_exit_without_commit_rolls_back_everything(
        self, session_factory: async_sessionmaker[AsyncSession]
    ):
        async with session_factory() as session:
            repository = SqlAlchemyUserRepository(session)
            ...
            async with transaction_context.begin():
                result, _ = await repository.create(
                    _make_user("alice", "alice@example.com")
                )
                assert result == operation_results.CreateResult.SUCCESS
    ...
    async def test_failed_operation_rolls_back_earlier_success(
        self, session_factory: async_sessionmaker[AsyncSession]
    ):
        ...
        async with session_factory() as session:
            ...
            async with transaction_context.begin() as transaction:
                first_result, _ = await repository.create(
                    _make_user("alice", "alice@example.com")
                )
                assert first_result == operation_results.CreateResult.SUCCESS

                second_result, _ = await repository.create(
                    _make_user("alice", "duplicate@example.com")
                )
                assert (
                    second_result
                ...
                if (
                    second_result == operation_results.CreateResult.SUCCESS
                ):  # pragma: no cover - documents the all-success gate
                    await transaction.commit()

        assert await _count_users(session_factory) == 0

    async def test_exception_inside_block_rolls_back_and_propagates(
        self, session_factory: async_sessionmaker[AsyncSession]
    ):
        async with session_factory() as session:
            repository = SqlAlchemyUserRepository(session)
    ...
    async def test_session_is_usable_after_rolled_back_block(
        self, session_factory: async_sessionmaker[AsyncSession]
    ):
