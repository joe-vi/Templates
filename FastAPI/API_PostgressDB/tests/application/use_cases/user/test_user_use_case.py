...
def use_case(
    ...
    return UserUseCase(repository=mock_repository, password_hasher=mock_password_hasher, transaction_context=fake_transaction_context)
...
class TestGetAllUsers:
    async def test_returns_dto_list_for_all_users(self, use_case, mock_repository):
...
class TestTransactionBoundaries:
    ...
    async def test_create_success_commits_transaction(self, use_case, mock_repository, fake_transaction_context):
        mock_repository.create.return_value = (operation_results.CreateResult.SUCCESS, 1)

        await use_case.create_user(user_dto_module.CreateUserDTO(email="a@e.com", username="a", password="TestPass123"))

        assert fake_transaction_context.transaction.is_committed is True
        assert fake_transaction_context.is_rolled_back is False

    async def test_create_failure_rolls_back_without_commit(self, use_case, mock_repository, fake_transaction_context):
        mock_repository.create.return_value = (operation_results.CreateResult.UNIQUE_CONSTRAINT_ERROR, None)
    ...
    async def test_update_role_success_commits_transaction(self, use_case, mock_repository, fake_transaction_context):
        mock_repository.update_role.return_value = operation_results.UpdateResult.SUCCESS

        await use_case.update_user_role(user_dto_module.UpdateUserRoleDTO(user_id=1, role=user_enum.UserRole.ADMIN))

        assert fake_transaction_context.transaction.is_committed is True

    async def test_update_role_not_found_rolls_back_without_commit(self, use_case, mock_repository, fake_transaction_context):
        mock_repository.update_role.return_value = operation_results.UpdateResult.NOT_FOUND

        await use_case.update_user_role(user_dto_module.UpdateUserRoleDTO(user_id=99, role=user_enum.UserRole.ADMIN))

        assert fake_transaction_context.transaction.is_committed is False
        assert fake_transaction_context.is_rolled_back is True

    async def test_delete_success_commits_transaction(self, use_case, mock_repository, fake_transaction_context):
        mock_repository.delete.return_value = operation_results.DeleteResult.SUCCESS
    ...
    async def test_delete_failure_rolls_back_without_commit(self, use_case, mock_repository, fake_transaction_context):
