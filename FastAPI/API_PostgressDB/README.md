...
│   │   ├── routers/                        # One route module per operation; each entity's router.py defines
│   │   │   ├── auth/                       # the shared APIRouter (prefix, tags, JWT guard declared once)
│   │   │   │   ├── login_route.py          # Routes import router.py and attach directly, take/return DTOs
│   │   │   │   ├── refresh_token_route.py
│   │   │   │   └── router.py               # shared APIRouter; __init__.py stays empty
│   │   │   └── user/
...
### 4. API Layer (`src/api/`)
- **Routes**: One FastAPI route module per operation, each depending on its use case via `Annotated[CreateUserUseCase, Injected(CreateUserUseCase)]` and **returning response models** (FastAPI serialises them to camelCase); each imports and attaches directly to the shared `APIRouter` defined once in the entity's `router.py` (prefix, tags, and JWT guard declared there; `__init__.py` stays empty)
...
4. **API**: Add one route module per operation in `src/api/routers/<name>/`, each importing and attaching to the shared `APIRouter` defined once in `router.py` (prefix, tags, guard declared there); routes depend on their use case and accept/return the DTOs directly (`response_model=<Entity>DTO`) — no per-entity schemas or converters
