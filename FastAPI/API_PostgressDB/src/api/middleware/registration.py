...
def register(app: FastAPI) -> None:
    ...
    needs the scope already open. ``exception_handler`` is innermost, so the 500 it returns for a
    failed request still travels back out through ``access_log``, which records it, and through
