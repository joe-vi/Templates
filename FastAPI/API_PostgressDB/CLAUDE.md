...
- Routes/guards resolve via `use_case: Injected[CreateUserUseCase]` — statically the identity alias `type Injected[T] = T` (the parameter's type IS the concrete class); at runtime `__class_getitem__` builds `Annotated[X, <Depends>]`, a thin Depends over `app.state.injector`.
