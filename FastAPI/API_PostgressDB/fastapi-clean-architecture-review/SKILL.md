...
- **DI**:
  ...
  - Each route module covers one operation and depends on its use case via `Annotated[CreateUserUseCase, Injected(CreateUserUseCase)]`; the entity's `router.py` aggregates them (prefix, tags, guard declared once) and `__init__.py` stays empty
