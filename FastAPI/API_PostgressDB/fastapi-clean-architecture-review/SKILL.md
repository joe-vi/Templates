...
- **DI**:
  ...
  - Each route module covers one operation with its own `APIRouter()` and **resource-relative paths** (`""` for the collection root, `/{id}` for item routes — flag any operation file that repeats the `/<entity>` segment in its path); it depends on its use case via `Annotated[CreateUserUseCase, Injected(CreateUserUseCase)]`; the entity's `router.py` aggregates them via `include_router(op.router, prefix="/<entity>")` so the resource segment is declared once there (version prefix, tags, guard on the router); `__init__.py` stays empty
